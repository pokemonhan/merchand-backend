      ...
      <div class="dia-inner">
        <div class="flex">
          <ul class="form">
            <li>
              <span>入款方式:</span>
              <Select style="width:250px;" v-model="form.inconm" :options="inconm_opt"></Select>
            </li>
          ...
          <ul class="form">
            ...
            <li @click.stop>
              <p v-show="tag_show" v-for="(item) in all_tag" :key="item.value" class="allTag-list">
                <input type="checkbox" v-model="form.formtag[item.value]" @change="tagChange(item)" />
                {{item.label}}
              </p>
            </li>
            <li class="declare">
